...
class Booking:
    ...
    def book_venue():
        ...
        headers = Booking._create_headers(serviceid)
        candidates = Booking._booking_candidates()
        ...
        for idx, cand in enumerate(candidates, start=1):
            ...
            payload = Booking._build_payload(cand, serviceid)

            for attempt in range(1, MAX_RETRY_PER_CANDIDATE + 1):
                encoded_payload = Booking._encode_payload(payload)
                ...
                if '未到该日期的预订时间' in message:
                    print(f"预约失败：{message}，将在当前会话内继续尝试 ({attempt}/{MAX_RETRY_PER_CANDIDATE})")
                    time.sleep(RETRY_SLEEP_SECONDS)
                    continue
                ...
                retryable = Booking._is_retryable_message(message)
                non_retryable = Booking._is_non_retryable_message(message)
